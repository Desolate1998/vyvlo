...
export const EditCategory: FC<IProps> = ({ open, setIsOpen, category }) => {
    const [name, setName] = useState<ValueError<string>>({ value: category.name, error: '' })
    const [description, setDescription] = useState<ValueError<string>>({ value: category.description, error: '' })
    const [tags, setTags] = useState<ValueError<string[]>>({ value: category.metaTags, error: '' })
    ...
    const onSubmit = async () => {
        ...
        try {
            let response = await request.patch<ErrorOr<boolean>>('/api/ManageStore/UpdateCategory', {
                name: name.value,
